...
class OuvrageRepository extends ServiceEntityRepository
{
    ...
    public function save(Ouvrage $ouvrage, $flush = true)
    {
        $this->manager->persist($ouvrage);
        if ($flush) {
            $this->manager->flush();
        }
    }

    public function remove(Ouvrage $ouvrage, $flush = true)
    {
        $this->manager->remove($ouvrage);
        if ($flush) {
            $this->manager->flush();
        }
    }
    public function findAllByAuteur($auteurId)
{
